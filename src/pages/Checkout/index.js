...
const columns = [
  { 
    ...
    accessor: item => <div className="flex items-center">
      <img src={`http://localhost:3000/images/products/${item.image_url}`} width={48} alt={item.name}/>
      {item.name}
    </div>
  },
  {
        ...
        <div>
         <Table
           ...
           items={[
             ...
             {label: 'Ongkir', value: formatRupiah(20000)}, 
             {label: 'Total', value: <b>{formatRupiah(sumPrice(cart) + 20000)}</b>}, 
           ]}
           showPagination={false}
         />
        <br />
